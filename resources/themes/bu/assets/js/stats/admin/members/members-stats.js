Vue.component('members-stats', {
            ...
            'columnConfigs': [
                {
                    ...
                    'dataPath': 'subscription.plan.name',
                    'default': 'None',
                    'sortOrder': 1
                },
                {
                    ...
                    'title': 'Installments',
                    'aggregationType': aggregationTypes.group,
                    'dataPath': 'subscription.installments.status',
                    'default': 'None',
                    'sortOrder': 2
                },
                {
                    ...
                    'dataPath': 'subscription.installments.total',
                    'default': 0,
                    'sortOrder': 3
                },
                {
                    ...
                    'aggregationType': aggregationTypes.sum,
                    'dataPath': 'subscription.installments.balance',
                    'default': 0,
                    'sortOrder': 4
                },
                {
    ...
    computed: {
        ...
        rows: function () {
            var vm = this;
            var rows = [];
            _.forEach(vm.members, function (member, memberKey) {
                var row = {'columns': []};
                _.forEach(vm.columnConfigs, function (columnConfig, columnKey) {
                    var data = vm.getNestedProperty(member, columnConfig.dataPath, columnConfig.default);
                    row.columns.push({
                        'id': columnConfig.id,
                        'title': columnConfig.title,
                        'data': data,
