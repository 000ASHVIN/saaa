...
<div class="col-sm-12 col-xs-12 app-plan app-plan-@{{ plan.id }}" style="margin-bottom: 20px;">
    ...
    line-height: 60px;">R</sup>@{{ plan.price }}</small>
        ...
        <div style="min-height: 44px;">
            ...
            <br>
            <small style="color: red; font-weight: bold">Save <sup>R</sup> @{{ plan.bf_price }}</small>
        </div>
        ...
        @if(auth()->user())
            @if(auth()->user()->subscribed('cpd') && auth()->user()->subscription('cpd')->plan->price > 0)
                ...
                {{--</div>--}}
                <div>
                    <a href="#" data-target="#need_help_subscription" data-toggle="modal" class="btn btn-3d btn-primary btn-block" style="text-transform: uppercase; font-size: 14px">
                        <i class="et-trophy"></i>
                        <span>
                        Change Plan
                    </span>
