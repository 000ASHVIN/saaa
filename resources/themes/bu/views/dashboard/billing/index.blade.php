...
@section('title', 'My Billing Information')
...
<section>
    <div class="container">
        @include('dashboard.includes.sidebar')
        	<app-my-billing-screen :user="{{ $user }}" inline-template>
			...
			<div class="col-lg-9 col-md-9 col-sm-8">
				...
				<div class="form-group">
					...
					<select name="payment_method" id="payment_method" class="form-control payment_method" v-model="payment_method">
						<option @if($user->payment_method == "") selected @endif value="">Please select..</option>
						<option @if($user->payment_method == "debit_order") selected @endif value="debit_order">Debit Order</option>
						<option @if($user->payment_method == "credit_card") selected @endif value="credit_card">Credit Card</option>

						@if($user->subscribed('cpd') && $user->subscription('cpd')->plan->interval == 'year')
							<option @if($user->payment_method == "eft") selected @endif value="eft">Electronic funds transfer (EFT)</option>
				...
				<div class="panel panel-default" v-if="payment_method == 'debit_order' && !waiting_on_otp">
					...
					<div class="panel-body">
						...
						<div class="form-group @if ($errors->has('account_holder')) has-error @endif">
							...
							{!! Form::input('text', 'account_holder', auth()->user()->debit? auth()->user()->debit->account_holder : null, ['class' => 'form-control', 'v-model' => 'forms.debit_order.debit.account_holder']) !!}
						...
						<div class="form-group">
							...
							<select name="type_of_account" v-model="forms.debit_order.debit.type_of_account" class="form-control">
								<option value="company" @if(auth()->user()->debit && auth()->user()->debit->type_of_account == 'company') selected @endif>Company Account</option>
								<option value="personal" @if(auth()->user()->debit && auth()->user()->debit->type_of_account == 'personal') selected @endif>Personal Account</option>
							</select>
						</div>

						<div class="form-group @if ($errors->has('id_number')) has-error @endif" v-if="forms.debit_order.debit.type_of_account == 'personal'">
							{!! Form::label('id_number', 'ID Number') !!}
							{!! Form::input('text', 'id_number', auth()->user()->id_number? auth()->user()->id_number : null, ['class' => 'form-control', 'v-model' => 'forms.debit_order.debit.id_number']) !!}
							@if ($errors->has('id_number')) <p class="help-block">{{ $errors->first('id_number') }}</p> @endif
						</div>

						<div class="form-group @if ($errors->has('registration_number')) has-error @endif" v-if="forms.debit_order.debit.type_of_account == 'company'">
							{!! Form::label('registration_number', 'Company Registration Number') !!}
							{!! Form::input('text', 'registration_number', auth()->user()->debit? auth()->user()->debit->registration_number : null, ['class' => 'form-control', 'v-model' => 'forms.debit_order.debit.registration_number']) !!}
							@if ($errors->has('registration_number')) <p class="help-block">{{ $errors->first('registration_number') }}</p> @endif
						</div>
						
						<div class="form-group">
							{!! Form::label('bank', 'Bank Name') !!}
							{!! Form::input('text','bank',auth()->user()->debit? auth()->user()->debit->bank : null, ['class' => 'form-control', 'v-model' => 'forms.debit_order.debit.bank']) !!}
						</div>
						...
						<div class="form-group">
							...
							{!! Form::input('text', 'branch_code', auth()->user()->debit? auth()->user()->debit->branch_code : null, ['class' => 'form-control', 'v-model' => 'forms.debit_order.debit.branch_code']) !!}
						</div>
						<div class="form-group">
							{!! Form::label('number', 'Account Number') !!}
							{!! Form::input('text', 'number', auth()->user()->debit? auth()->user()->debit->number : null, ['class' => 'form-control', 'v-model' => 'forms.debit_order.debit.number']) !!}
						</div>

						<div class="form-group">
							{!! Form::label('branch_name', 'Branch Name') !!}
							{!! Form::input('text', 'branch_name', auth()->user()->debit? auth()->user()->debit->branch_name : null, ['class' => 'form-control', 'v-model' => 'forms.debit_order.debit.branch_name']) !!}
						</div>
