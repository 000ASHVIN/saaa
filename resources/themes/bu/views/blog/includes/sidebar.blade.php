    ...
    <div class="side-nav margin-bottom-60 margin-top-30">
        ...
        <ul class="list-group list-group-bordered list-group-noicon uppercase">
            @foreach($categories as $category)
                <li class="list-group-item {{ isActive('news/'.$category->slug, true) }}"><a href="{{ route('news.index', $category->slug) }}"><span class="size-11 text-muted pull-right">({{ count($category->posts) }})</span>{{ $category->title }}</a></li>
            @endforeach
