...
@if((auth()->user()->subscribed('cpd') && auth()->user()->subscription('cpd')->plan->price != '0') || (auth()->user()->hasCompany() && auth()->user()->hasCompany()->company && auth()->user()->hasCompany()->company->admin()->subscription('cpd') && auth()->user()->hasCompany()->company->admin()->subscription('cpd')->plan->price != '0'))
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <a href="{{ route('resource_centre.legislation.create') }}" class="btn btn-default" style="color: white; background-color: #800000; border-color: #800000;"><i class="fa fa-comment-o"></i> Start New Conversation</a>
                </div>
            </div>
