...
Vue.component('app-single-subscriptions-screen', {
    ...
    ready() {
        ...
        var Selectplans = $.cookie('plan');
        var selectedP = _.filter(this.plans, function (plan){
             return plan.id == Selectplans;
        });
        if(selectedP.length>0){
        this.selectPlan(selectedP[0]);
        $.cookie('plan',null);
        $.cookie('pathName',null);
        }
    ...
    methods: {
        ...
        getEftLink: function () {
            ...
            this.$http.post('/instant-eft', { staffs:this.forms.subscription.staff, plan: this.selectedPlan.id, bf: this.forms.subscription.bf })
                .success((response) => {
        ...
        updateDebitOrderDate: function (){
            var d = new Date();
            var day = d.getDate();
            if(this.forms.subscription.debit.billable_date < day)
            {
                  swal({
                    title: "Debit Order",
                    text: "Due to your selected date is past away. So, Your debit order will be run next month your selected date Wait until the debit order runs, (you will need to wait up to 5 working days to get access once the debit order has successfully run for current month \n Please select next day debit order to get instant access or Select credit card or Instant EFT payment to get instant access",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#007AFF",
                    confirmButtonText: "Next Day Debit Order!",
                    cancelButtonText: "Select Other Payment Method",
                    closeOnConfirm: true
                }, function() {
                    $('input[name=bill_at_next_available_date]').val("1").trigger('change');    
                   
                });
            }else{
                swal({
                    title: "Debit Order",
                    text: "Wait until the debit order runs on your selected date, (you will need to wait up to 5 working days to get access once the debit order has successfully run for current month \n Please select next day debit order to get instant access or Select credit card or Instant EFT payment to get instant access",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#007AFF",
                    confirmButtonText: "Next Day Debit Order!",
                    ...
                    $('input[name=bill_at_next_available_date]').val("0").trigger('change');    
