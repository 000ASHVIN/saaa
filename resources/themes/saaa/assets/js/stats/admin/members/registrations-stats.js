Vue.component('registrations-stats', {
        ...
        return {
            ...
            'columnConfigs': [
                ...
                },
                {
                    ...
                    'title': 'Subscription',
                    'aggregationType': aggregationTypes.group,
                    'dataPath': 'user.subscription.plan.name',
                    'default': 'None',
                    'sortOrder': 4
                },
                {
    ...
    computed: {
        ...
        rows: function () {
            var vm = this;
            var rows = [];
            _.forEach(vm.registrations, function (registration, registrationKey) {
                var row = {'columns': []};
                _.forEach(vm.columnConfigs, function (columnConfig, columnKey) {
                    var data = vm.getNestedProperty(registration, columnConfig.dataPath, columnConfig.default);
                    row.columns.push({
                        'id': columnConfig.id,
                        'title': columnConfig.title,
                        'data': data,
