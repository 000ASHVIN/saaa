Vue.component('app-admin-order-pay', {
    ...
    ready: function() {
        ...
        if(this.order.items.length > 0){
            this.amount = parsefloat(this.order.items[0].price) * parseInt(this.order.items[0].quantity );
            this.pricedeault = this.order.items[0].price;
        }
     ...
     watch: {
        selectedStaffLength: function(val){
            if(val > 0)
            {
                //this.amount = parseFloat(this.pricedeault) * (parseInt(val)+parseInt(1));
            }
    ...
    methods: {
        ...
        sendInvitation(){
            ...
            this.$http.post('/dashboard/company/store_invite_company',{first_name:this.company.invite.first_name,
        ...
        getPlanPrice: function()
        {
            if(this.selectedplan.pricing_group.length>0)
            {
                ...
                if(minuser.length>0)
                {
                    ...
                    if(staff.length > minuserCount) 
                    {
                        return minuser;
                    }
                }
                if(staff.length > plans) 
                {
                ...
                return _.filter(this.selectedPlan.pricing_group, function (group) {
                    return parseInt(group.min_user) <= parseInt(staff.length) && parseInt(group.max_user) >= parseInt(staff.length);
