...
@section('content')
    {{-- <section id="slider" class="hidden-sm hidden-xs">
        <center style="background-image: url('https://imageshack.com/a/img923/8140/VeRFYZ.jpg')">
            <div style="background-image: url('https://imageshack.com/a/img923/8140/VeRFYZ.jpg'); height: 320px; background-color: #000000; position:relative; top: 55px;">
    ...
    <section style="background:url('/assets/frontend/images/demo/wall2.jpg')">
        ...
        <app-single-subscriptions-screen  :subscriptions="{{(auth()->user()?auth()->user()->subscription('cpd')->plan:auth()->user())}}"   :plans="{{ $plans }}" :user="{{ (auth()->user() ? auth()->user()->load(['cards', 'subscriptions']) : auth()->user()) }}" inline-template>
            <div id="app-register-screen" class="container app-screen">
                @if (Route::getFacadeRoot()->current()->uri() == 'subscriptions/2020/BlackFriday'  || Route::getFacadeRoot()->current()->uri() == 'subscriptions/2020/one-day-only'  || Route::getFacadeRoot()->current()->uri() == 'BlackFriday')
            ...
            <br>
            <br>
        </app-single-subscriptions-screen>
    </Section>
