...
@if(in_array('1',$question))
<div class="form-group @if ($errors->has('name')) has-error @endif">
    {!! Form::label('name', 'Name') !!}
    {!! Form::input('text', 'name', auth()->user()?auth()->user()->name:null, ['class' => 'form-control']) !!}
    @if ($errors->has('name')) <p class="help-block">{{ $errors->first('name') }}</p> @endif
</div>
@endif

@if(in_array('2',$question))
...
@else
<input type="hidden" name="product" value="{{$content->slug}}">
@endif

@if(in_array('5',$question))
