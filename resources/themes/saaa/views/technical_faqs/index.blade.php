...
@if((auth()->user()->subscribed('cpd') && auth()->user()->subscription('cpd')->plan->price != '0') || (auth()->user()->hasCompany() && auth()->user()->hasCompany()->company && auth()->user()->hasCompany()->company->admin()->subscription('cpd') && auth()->user()->hasCompany()->company->admin()->subscription('cpd')->plan->price != '0'))
    ...
    <section>
        <div class="container">
            @foreach($faqCategories->chunk(3) as $chunk)
                <div class="row">
                    @foreach($chunk as $folder)
                        <div class="col-md-4">
                            <div class="border-box margin-bottom-10" style="background-image: url('/assets/frontend/certificates/cpd/images/bg.jpg'); background-size: cover; text-align: center; min-height: 200px; display: -webkit-flex; -webkit-align-items: center; border: 1px solid #173175 ;">
                                <div class="center-block">
                                    <h4 style="vertical-align: center; width: 245px; color: #173175; min-height: 45px">{{ ucwords($folder->name) }}</h4>
                                    <hr>
                                    <p style="vertical-align: center; width: 250px; color: #173175; min-height: 100px">{{ $folder->description }}</p>
                                    <hr>
                                    <div class="form-inline">
                                        <button style="background: white; color: #173175;" class="btn btn-primary">{{ $folder->faqs_count() }} Articles</button>
                                        <a href="{{ route('resource_centre.technical_faqs.show', $folder->id) }}" class="btn btn-primary">View FAQs <i class="fa fa-arrow-right"></i></a>
                                    </div>
