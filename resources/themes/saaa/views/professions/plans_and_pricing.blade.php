...
<app-subscriptions-screen-plan :staff="{{ $staff }}" :companys="{{ $company }}" :plans="{{ $business }}" :profession="{{ $profession }}"
        :subscriptions="{{(auth()->user()?auth()->user()->subscription('cpd')->plan:auth()->user())}}"
            :user="{{ (auth()->user() ? auth()->user()->load('cards') : auth()->user()) }}" :bank_codes="{{ json_encode(getBankBranches()) }}" 
            :donations="{{ env('DONATIONS_AMOUNT') }}"
            :before_selected_plan="{{ $selectedPlan ?? null }}" inline-template>  
    ...
    <ul class="nav nav-tabs">
            ...
            <li><a class="tab_li" @click.prevent="makeactive('trainee','Trainee-Accountant')"  data-toggle="tab" href="#students" >Trainee Subscription Plans</a></li> 
    </ul>
    ...
    <div class="tab-content">  
        ...
        <div id="students" class="tab-pane fade">
            <h3>Trainee Subscription Plans</h3>
...
<script>
    ...
    function selectNavTab(number, id) {
        $('.nav-tabs > li.active').removeClass('active')
        $('.nav-tabs > li:nth-child('+number+')').addClass('active')
        
        $('.tab-pane.active').removeClass('active')
        $(id).addClass('in active')
    }
</script>
@if ($selectedPlan && $selectedPlan->package_type == "business")
<script>
    selectNavTab(2, '#business');
</script>   
@endif

@if ($selectedPlan && $selectedPlan->package_type == "trainee")
<script>
    selectNavTab(3, '#students');
</script>   
@endif
@stop