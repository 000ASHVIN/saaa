...
@section('content')
    <section>
        <div class="container">
            ...
            <p>
                If you have any queries regarding your outstanding balance please contact our accounts department on 012 943 7002 or send an email to
                {{ config('app.email') }}.
                    ...
                    @if(count($user->overdueInvoices()) >= 1)
                        @foreach($user->overdueInvoices() as $invoice)
                            <tr>
                                ...
                                <td>{{ money_format('%.2n',  $invoice->transactions->where('type', 'debit')->sum('amount') - $invoice->transactions->where('type', 'credit')->sum('amount'))  }}</td>
