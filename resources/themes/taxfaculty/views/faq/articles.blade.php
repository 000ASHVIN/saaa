                        ...
                        <ul class="nav mix-filter" style="text-transform: capitalize;">
                            @if(count($categories))
                                @foreach($categories->unique() as $tag)
                                    <li style="background-color: #8cc03c!important; margin-top: 10px; margin-bottom: 10px" data-filter="{{ $tag }}" class="filter"><a style="color: #ffffff" id="faqs_tags" href="#">{{ $tag }}</a></li>
                                @endforeach
                            @else
