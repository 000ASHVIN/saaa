        ...
        <div class="container g-z-index-1 g-py-100 event-inner-banner" style="background-color: rgba(255, 255, 255, 0.8784313725490196)!important;">
            ...
            <div class="row">
                ...
                <div class="col-md-3 col-md-offset-2">
                    ...
                    <p style="font-size: 16px"><i class="fa fa-tag"></i> <strong>Application fee: </strong> R{{ number_format($course->monthly_enrollment_fee, 2) }}</p>
                    <p style="font-size: 16px"><i class="fa fa-tag"></i> <strong>Once-off: </strong> R{{ number_format($course->yearly_enrollment_fee, 2) }}</p>
                </div>
                @else    
                <div class="col-md-4 col-md-offset-2">
                    
                    <p style="font-size: 16px"><i class="fa fa-calendar"></i> <strong>Start date:</strong> {{ $course->start_date->toFormattedDateString() }}</p>
                    <p style="font-size: 16px"><strong>Payment options per semester: </strong></p>

                    @if(in_array($course->id,$array))
                    <p style="font-size: 16px"> <strong>Pre Approval Course Fee: </strong> R{{ number_format($course->monthly_enrollment_fee, 2, '.', ' ') }}</p>
                    @endif
                    <p style="font-size: 16px"><strong>Payment option 1: Once-off payment: </strong></p>
                   
                    <div class="payment-details">
                        <p style="font-size: 16px"> Full course fee: R{{ number_format((float)$course->semester_price * (float)$course->no_of_semesters, 2, '.', ' ') }}</p>
                        <p style="font-size: 16px"> First semester fee: R{{ number_format((float)$course->semester_price, 2, '.', ' ') }}</p>
                    </div>

                    @if($course->monthly_enrollment_fee > 0 && !in_array($course->id,$array))
                    {{-- <p style="font-size: 16px"><strong>Monthly : </strong></p>
                    <p style="font-size: 16px"> <strong>Application fee: </strong> R {{ number_format($course->monthly_enrollment_fee, 2) }}</p>
                    <p style="font-size: 16px">R {{ number_format($course->recurring_monthly_fee, 2) }} * {{$course->no_of_debit_order}} Monthly installments</p> --}}
                    <p style="font-size: 16px"><strong>Payment option 2: Debit order </strong></p>
                    <div class="payment-details">
                        <p style="font-size: 16px"> Application fee: R{{ number_format($course->monthly_enrollment_fee, 2, '.', ' ') }}</p>
                        <p style="font-size: 16px"> First payment upon approval: R{{ number_format((float)$course->order_price, 2, '.', ' ') }}</p>
                        <p style="font-size: 16px"> Monthly fee: R{{ number_format($course->recurring_monthly_fee, 2, '.', ' ') }} per month, five instalments per semester</p>
                    </div>
...
{{--                            </div>--}}
                    ...
                    <div class="custom-md-4" style="padding: 15px">
                        ...
                        @if ($course->brochure)
                            <button type="button" data-cid="{{$course->id}}"  data-target="#course_brochure_popup" class="btn btn-primary download_brochure form-control" style="margin: 15px 0px;">Download Course Brochure</button>
