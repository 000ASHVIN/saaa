...
|| Route::getFacadeRoot()->current()->uri() == 'BlackFriday' || Route::getFacadeRoot()->current()->uri() == 'subscriptions/2020/one-day-only')
    <!-- Default / Disabled Monthly Plans -->
    <div class="row" v-if="defaultPlans.length > 0 && shouldShowDefaultPlans">
        <div class="col-md-4 " v-for="plan in defaultPlans">
    ...
    <div class="row" v-if="yearlyPlans.length > 0 && shouldShowYearlyPlans">
        <div class="col-md-4 " v-for="plan in yearlyPlans">
            @include('subscriptions.partials.plans.disabled_plans')
        </div>
    </div>
@else
    ...
    <div class="row" v-if="defaultPlans.length > 0 && shouldShowDefaultPlans">
        <div class="col-md-4 " v-for="plan in defaultPlans">
            @include('subscriptions.partials.plans.pricingplan')
        </div>
    </div>
