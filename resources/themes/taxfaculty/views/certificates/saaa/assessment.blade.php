...
<head>
    ...
    <style>
        ...
        table.certificate {
            ...
            height: 287mm;
            @if($pdf)
                background: url('{{asset("assets/frontend/certificates/cpd/images/bg.png")}}') no-repeat;
            @else
                background: url('{{asset("assets/frontend/certificates/cpd/images/bg.png")}}') no-repeat;
            @endif
            background-size: 100%;
        ...
        td.hours-cpd {
            @if($pdf)
                background: url('{{asset('assets/frontend/certificates/cpd/images/badge-top.png')}}') no-repeat bottom center;
            @else
                background: url('{{asset('assets/frontend/certificates/cpd/images/badge-top.png')}}') no-repeat bottom center;
            @endif
            width: 183px;
        ...
        td.hours-cpd-title {
            @if($pdf)
                background: url('{{ asset('assets/frontend/certificates/cpd/images/badge-bottom.png') }}') no-repeat top center;
            @else
                background: url('{{ asset('assets/frontend/certificates/cpd/images/badge-bottom.png') }}') no-repeat top center;
            @endif
            width: 183px;
...
<table class="certificate" width="100%">
    <tr style="height: 140px;">
        <td class="certificate-logo">
            @if($pdf)
                <img src="{{ asset('assets/frontend/certificates/cpd/images/logo.png') }}"
                     alt="{{settings('company-name')}}">
            @else
                <img src="{{ asset('assets/frontend/certificates/cpd/images/logo.png') }}" alt="{{settings('company-name')}}">
            @endif
        </td>
    </tr>
    <tr>
            ...
            <table class="sub-table">
                <tr>
                    ...
                    <td class="presenter-and-date">
                        ...
                        @if (count($cpd->certificate->source->events()->first()->presenters))
                            <div class="space-2"></div>
                            <div class="presenter">
