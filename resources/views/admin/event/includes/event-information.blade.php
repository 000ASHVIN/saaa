...
<div class="col-md-12">
    <div class="row">
        <div class="col-md-6">
            <div class="form-group @if ($errors->has('sync_event')) has-error @endif">
                {!! Form::label('sync_event', 'Synced Tax Faculty subscription Event?') !!}
               
                {!! Form::select('sync_event', ['yes' => 'Yes', 'no' => 'No'], $event->is_synced_event ? 'yes' : 'no', ['class' => 'form-control']) !!}
                
                @if ($errors->has('sync_event')) <p class="help-block">{{ $errors->first('sync_event') }}</p> @endif
            </div>
        </div>
    </div>
    <hr>
