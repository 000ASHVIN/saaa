...
class GenerateFreeSubscriptionTickets extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'generate:free-subscriber-tickets';
     ...
     * @var string
     */
    protected $description = 'This command will assign all the Free Subscription events to the subscribes';
    ...
    public function handle()
    {
        ...
        foreach ($plans as $plan){
            ...
            $subscribers = $this->getSubscribers($plan,$events);
    ...
    public function getSubscribers($plan,$events)
    {
        $users = collect([]);

        $subscriptions = User::select('users.*')->with('employing_companies')->join('subscriptions', function($join) use($plan)
            {
            ...
            ->groupBy('subscriptions.user_id')->chunk(1000, function ($users) use($events) {
