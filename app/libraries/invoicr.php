...
class invoicr extends FPDF_rotation
{
    ...
    var $margins = array('l' => 20, 't' => 30, 'r' => 20);
    var $snappymargins = array('l' => 174, 't' => 65, 'r' => 0);
    var $accountabilityMargins = array('l' => 24, 't' => 20, 'r' => 20);
    ...
    function setSnappy($image = 0, $maxWidth = 0, $maxHeight = 0)
    {
        if ($maxWidth and $maxHeight) {
            $this->maxImageDimensions = array($maxWidth, $maxHeight);
        }
        $this->snapScan = $image;
        $this->dimensions = $this->resizeSnappyToFit($image);
    }

    function setAccountability($image = 0, $maxWidth = 0, $maxHeight = 0)
    {
        if ($maxWidth and $maxHeight) {
            $this->maxImageDimensions = array($maxWidth, $maxHeight);
        }
        $this->accountability = $image;
        $this->dimensions = $this->resizesetAccountability($image);
    }
    ...
    function addAdjustment($type, $description, $quantity, $vat, $price, $discount = 0, $total)
    {
        $adjustment['item'] = $type;
        $adjustment['description'] = $this->br2nl($description);
        $adjustment['vat'] = $vat;
        if (is_numeric($vat)) {
            $adjustment['vat'] = $this->currency . ' ' . number_format($vat, 2, $this->referenceformat[0], $this->referenceformat[1]);
        }
    ...
    function addAdjustmentTotal($name, $value, $colored = 0)
    {
        $t['name'] = $name;
        $t['value'] = $value;
        if (is_numeric($value)) {
    ...
    function addBadge($badge, $badgeColor = '#fcbd0f')
    {
        $this->badgeColor = $this->hex2rgb($badgeColor);
        $this->badge = $badge;
    }
    ...
    function Body()
    {
        if (isset($this->snapScan)){
            $this->Image($this->snapScan, $this->snappymargins['l'], $this->snappymargins['t'], $this->dimensions[20], $this->dimensions[1]);
        };
        if (isset($this->accountability)){
            $this->Image($this->accountability, $this->accountabilityMargins['l'], $this->accountabilityMargins['t'], $this->dimensions[20], $this->dimensions[1]);
        };
        $width_other = ($this->document['w'] - $this->margins['l'] - $this->margins['r'] - $this->firstColumnWidth - ($this->columns * $this->columnSpacing)) / ($this->columns - 1);
        $cellHeight = 9;
            ...
            foreach ($this->items as $item) {
                ...
                if ($this->discountField) {
                    ...
                    $this->Cell($width_other, $cHeight, iconv('UTF-8', 'windows-1252', $item['discount']), 0, 0, 'C', 1);
                        ...
                        $this->Cell($width_other, $cHeight, iconv('UTF-8', 'windows-1252', $this->currency . ' 0.00'), 0, 0, 'C', 1);
                }
                $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
    ...
    function adjustmentBody()
    {
        ...
        //Add totals
        if ($this->AdjustmentTotals) {
            foreach ($this->AdjustmentTotals as $total) {
                $this->SetTextColor(50, 50, 50);
                $this->SetFillColor($bgcolor, $bgcolor, $bgcolor);
                $this->Cell(1 + $this->firstColumnWidth, $cellHeight, '', 0, 0, 'L', 0);
                for ($i = 0; $i < $this->columns - 3; $i++) {
    ...
    private function resizeSnappyToFit($image)
    {
//        $snappy_width = 150;
//        $snappy_height = 150;
//        $newWidth = $this->maxImageDimensions[0] / $snappy_width;
//        $newHeight = $this->maxImageDimensions[1] / $snappy_height;
//        $scale = min($newWidth, $newHeight);
//        return array(
//            round($this->pixelsToMM($scale * $snappy_width)),
//            round($this->pixelsToMM($scale * $snappy_height))
//        );
    }

    private function resizesetAccountability($image)
    {
//        $snappy_width = 150;
//        $snappy_height = 150;
//        $newWidth = $this->maxImageDimensions[0] / $snappy_width;
